...
module Fluent
  class GeoIPFilter < Filter
	...
	config_param :locale2, :string, :default => DEFAULT_LOCALE_2,
                 :desc => 'Get the data for the specified locale (default: %s).' % DEFAULT_LOCALE_2
      ...
      unless ip.nil? then
        ...
        if geoip.found? then
          ...
          if @continent then
			...
			unless geoip.continent.name(@locale2).nil? then
              continent_hash['name'] = geoip.continent.name(@locale2)
            end
          ...
          if @country then
            ...
            unless geoip.country.iso_code.nil? then
              country_hash['code'] = geoip.country.iso_code
            end
            unless geoip.country.name(@locale2).nil? then
              country_hash['name'] = geoip.country.name(@locale2)
            end
          ...
          if @city then
            ...
            unless geoip.city.name(@locale2).nil? then
              city_hash['name'] = geoip.city.name(@locale2)
            end
            ...
            unless city_hash.empty? then
              if @flatten then
                ...
                record[@output_field].merge!({'city' => city_hash['name']})
              end
            end
          end
          ...
          if @location then
            ...
            unless geoip.location.latitude.nil? then
              location_hash['lat'] = geoip.location.latitude
            end
            unless geoip.location.longitude.nil? then
              location_hash['lon'] = geoip.location.longitude
            end
          ...
          if @subdivisions then
            ...
            geoip.subdivisions.each do |subdivision|
              ...
              unless subdivision.iso_code.nil? then
                subdivision_hash['code'] = subdivision.iso_code
              end
			  unless subdivision.name(@locale2).nil? then
                subdivision_hash['name'] = subdivision.name(@locale2)
              end
            ...
            unless subdivision_arry.empty? then
              if @flatten then
                ...
                record[@output_field].merge!({'subdivisions' => subdivision_arry[0]})
              end
            end
          end
